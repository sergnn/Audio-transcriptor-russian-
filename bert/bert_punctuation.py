...
class BertPunctuation(object):
    def __init__(self):
        self.model_file = "./bert/bert_punctuation.tar.gz"
        self.vocab_file = "./bert/vocab.txt"
    ...
    def predict(self, texts):
        ...
        for i, words in enumerate(words_all):
            ...
            choice_list = words.replace('–', '-').replace('—', '-').replace(',', ' ,').replace('.', ' .').replace('!', ' !').replace('?', ' ?').replace(':', ' :').replace(';', ' ;').replace('»',
                                                                                                                                                                                              ' »').replace(
                '«', '« ').split()
